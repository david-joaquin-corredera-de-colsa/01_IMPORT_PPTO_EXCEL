...
Public Sub M001_Ejecutar_Proceso_Principal()
    ...
    '--------------------------------------------------------------------------
    ' 6. NUEVA FUNCIONALIDAD AÑADIDA: Copiar datos de envío a comprobación
    '--------------------------------------------------------------------------
    lngLineaError = 97
    fun801_LogMessage "Iniciando copia de datos de envío a comprobación..."
    
    blnResult = F005_Copiar_Hoja_Envio_A_Comprobacion(gstrNuevaHojaImportacion_Envio, _
                                                      gstrNuevaHojaImportacion_Comprobacion)
    
    If Not blnResult Then
        Err.Raise ERROR_BASE_IMPORT + 1004, strFuncion, _
            "La copia de datos de envío a comprobación no se completó correctamente"
    End If
