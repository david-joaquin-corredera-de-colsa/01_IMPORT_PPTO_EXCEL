...
Public Function F005_Copiar_Hoja_Envio_A_Comprobacion(ByVal vHojaOrigen As String, _
    ...
    '******************************************************************************
    ' Función: F005_Copiar_Hoja_Envio_A_Comprobacion
    ' Fecha y Hora de Creación: 2025-06-01 19:53:00 UTC
    ' Autor: david-joaquin-corredera-de-colsa
    '
    ' Descripción:
    ' Copia todo el contenido de la hoja de envío a la hoja de comprobación
    ' para verificación y control de calidad. Esta función reutiliza las
    ' funciones auxiliares existentes en el proyecto.
    '
    ' RESUMEN EXHAUSTIVO DE PASOS:
    ' 1. Validación de parámetros de entrada (nombres de hojas)
    ' 2. Verificación de existencia de ambas hojas usando fun802_SheetExists
    ...
    '
    ' Parámetros:
    ' - vHojaOrigen (String): Nombre de la hoja de envío (origen)
    ' - vHojaDestino (String): Nombre de la hoja de comprobación (destino)
    '
    ' Retorna: Boolean (True si exitoso, False si error)
    '
    ' Compatibilidad: Excel 97, 2003, 365, OneDrive, SharePoint, Teams
    ...
    '--------------------------------------------------------------------------
    lngLineaError = 50
    fun801_LogMessage "Iniciando copia de envío a comprobación", False, "", ""
    
    ' Validar que los nombres de hojas no estén vacíos
    If Len(Trim(vHojaOrigen)) = 0 Then
        fun801_LogMessage "Error: Nombre de hoja origen vacío", True, "", ""
        Err.Raise ERROR_BASE_IMPORT + 1, strFuncion, _
            "El nombre de la hoja origen no puede estar vacío"
    End If
    
    If Len(Trim(vHojaDestino)) = 0 Then
        fun801_LogMessage "Error: Nombre de hoja destino vacío", True, "", ""
        Err.Raise ERROR_BASE_IMPORT + 2, strFuncion, _
            "El nombre de la hoja destino no puede estar vacío"
    End If
    ...
    fun801_LogMessage "Parámetros validados correctamente", False, "", ""
    
    '--------------------------------------------------------------------------
    ' 2. Verificación de existencia de hojas usando función existente
    '--------------------------------------------------------------------------
    lngLineaError = 70
    fun801_LogMessage "Verificando existencia de hojas", False, "", ""
    
    ' Verificar existencia de hoja origen usando función existente
    ...
    ' Verificar existencia de hoja destino usando función existente
    ...
    '--------------------------------------------------------------------------
    ' 3. Limpiar hoja de destino usando función existente
    '--------------------------------------------------------------------------
    lngLineaError = 90
    ...
    '--------------------------------------------------------------------------
    ' 4. Copiar contenido completo usando función auxiliar reutilizada
    '--------------------------------------------------------------------------
    lngLineaError = 110
    ...
    '--------------------------------------------------------------------------
    lngLineaError = 130
    fun801_LogMessage "Copia de envío a comprobación completada con éxito", False, "", ""
